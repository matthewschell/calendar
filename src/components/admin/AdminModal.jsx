      ...
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl text-center">
          ...
          <form onSubmit={handlePinSubmit}>
            <input type="password" autoComplete="new-password" value={pin} onChange={(e) => setPin(e.target.value)} maxLength={4} autoFocus className="w-full text-center text-3xl tracking-[1em] font-bold p-4 border-2 border-slate-200 rounded-xl mb-4 focus:border-indigo-500 focus:outline-none transition-colors" placeholder="••••" />
            ...
            {activeTab === 'system' && (
              <div>
                ...
                <div className="bg-white p-6 rounded-2xl border-2 border-slate-100 shadow-sm mt-4">
                  ...
                  <p className="text-sm text-slate-500 mb-4">Pull your old events from the Firebase Realtime Database and push them into the new Firestore system.</p>
                  <button onClick={handleMigrateLegacyEvents} disabled={isMigrating} className="py-2 px-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-colors disabled:opacity-50">
